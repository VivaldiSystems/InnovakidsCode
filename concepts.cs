...
public class Concepts : IConcepts
{
     ...
     public string[][] GetConcepts(string SchoolNo,string argFilter)
    {
         ...
         //    return GetLessonsDataString("Select LessonNo,Lesson,ChapterNo,Standard,CONVERT(VARCHAR(10),StartDate,110) As StartDate,Objective from Lesson where schoolno  = '" + SchoolNo + "' Order By LessonNo",1,10000);
           ...
           //  argFilter = argFilter.Replace("~","'");
             ...
             return GetConceptsDataString("Select ConceptNo,LessonNo,OrderNo,Concept,TextCaption,Text,StandardNo,FileName from Concept where lessonno = 2102 order by orderno",1,10000);
        ...
        foreach( System.Data.DataRow row in ds.Tables[0].Rows )//or similar
         {
           ...
           row[5] = row[5].ToString().Replace(",","~"); 
            ...
            strConcepts.Add( "<a href='javascript:OpenConceptEdit("+row[0].ToString()+");' >"+ row[0].ToString()+"</a>,"+row[2].ToString()+","+row[3].ToString()+","+row[4].ToString()+","+row[5].ToString() +","+row[6].ToString()  ); 
